/*
 * ═══════════════════════════════════════════════════════════════════════════
 * R2bot — 8-direction Stick + 2-motor D-Pad + 5 туслах мотор
 * ═══════════════════════════════════════════════════════════════════════════
 *
 *   START          → Toggle ON/OFF
 *
 ...
 *   R2 → M5  (D10/D11) PWM     cycle:  STOP→CW→STOP→CCW→STOP...   ⭐ШИНЭ⭐
 *   SELECT+START → M10 (D33/D32) cycle, LED on D34
 *   SELECT+R1    → M11 (D37) 270° servo toggle (HOME ↔ 270°)
 *
 *   M6 (D12/D13) — зарлагдсан ч АШИГЛАХГҮЙ
 *
 * ━━━ 🌀 PAIR DRIVE COMBO ━━━
 ...
 *
 * ━━━ 🧪 TEST COMBO ━━━
 *   SELECT + ×    → 4 мотор дарааллаар
 *   SELECT + △    → diagonal pair test
 *   ○             → test/cancel
 *
 * ━━━ 🔧 LIVE CALIBRATION ━━━
 *   L1+⬆/⬇ → M1 ±5    R1+⬆/⬇ → M2 ±5
 *   L2+⬆/⬇ → M4 ±5    R2+⬆/⬇ → M3 ±5     ⚠️ R2+⬆⬇ үед M5 cycle хийгдэхгүй
...
// Туслах моторын INVERT
const bool MOTOR_INVERT_M5  = false;   // R2 cycle (PWM)
...
#define M4_RPWM   8
#define M4_LPWM   9
// ▼ M5 — PWM (R2 cycle), M6 — PWM (АШИГЛАХГҮЙ) ▼
#define M5_RPWM   10
#define M5_LPWM   11
#define M6_RPWM   12
#define M6_LPWM   13
...
const int M5_SPEED        = 230;   // M5 cycle speed
...
// M5 циклийн state (4-state: 0=STOP, 1=CW, 2=STOP, 3=CCW)
int       m5State          = 0;   // R2 cycle
// M6 — declared but not used
int       m6State          = 0;
...
// ═══════════════════════════════════════════
//   R2 → 4 төлөв цикл: STOP → CW → STOP → CCW → STOP → ...
//   ⚠️ L2+R2 (test), R1+R2 (pair), R2+⬆⬇ (calib) combo үед cycle хийхгүй
// ═══════════════════════════════════════════
...
void handleM5() {
  bool comboActive = ps2x.Button(PSB_L2)         // L2+R2 = test
                  || ps2x.Button(PSB_R1)         // R1+R2 = pair mode
                  || ps2x.Button(PSB_PAD_UP)     // R2+⬆ = M3 calib
                  || ps2x.Button(PSB_PAD_DOWN);  // R2+⬇ = M3 calib
...
// ═══════════════════════════════════════════
// 🔁 M6 — ЗАРЛАГДСАН ГЭХДЭЭ АШИГЛАХГҮЙ
// (handleM6 ДУУДАГДАХГҮЙ — processInput-д комментолсон)
// ═══════════════════════════════════════════
...
angleDeg += 45.0f;                          // ← 45° шилжүүлсэн (F → FR)
...
void processInput() {
  ...
  // M6 — АШИГЛАХГҮЙ (зарлагдсан ч дуудагдахгүй)
  // handleM6();  ← idle, дуудахгүй

  // M5, M7-M11 идэвхтэй
  handleM5();    // R2 cycle (PWM мотор)              ⭐ШИНЭ⭐
...
void setup() {
  ...
  Serial.println(F("\n═══════════════════════════════════════"));
  Serial.println(F("  R2bot — 8-direction + 5 aux motors"));
  Serial.println(F("═══════════════════════════════════════\n"));
  ...
  Serial.println(F("  M5  (D10,D11) → R2 cycle (PWM мотор)"));
  Serial.println(F("  M6  (D12,D13) → ЗАРЛАГДСАН ГЭХДЭЭ АШИГЛАХГҮЙ"));
  ...
  Serial.println(F("  R2 → M5 cycle  (4-state: STOP/CW/STOP/CCW)  ⭐NEW⭐"));
