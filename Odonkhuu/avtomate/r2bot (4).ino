...
// ═══════════════════════════════════════════
//   SELECT+START → 4 төлөв цикл
//   LED (D34) — мотор асах үед ассан, зогссон үед унтарсан
// ═══════════════════════════════════════════
...
void handleM10() {
  // SELECT held + START rising edge → cycle
...
// ═══════════════════════════════════════════
// 🧪 MOTOR TEST COMBOS (TEST_SEQUENCE → SELECT+× руу нүүсэн)
// ═══════════════════════════════════════════
bool handleTestCombos() {
  // TEST SEQUENCE — SELECT + ×
  if (ps2x.Button(PSB_SELECT) && ps2x.ButtonPressed(PSB_CROSS)) {
...
void setup() {
  ...
  Serial.println(F("  R1 → M9 hold  (дараастай үед асна)"));
  ...
  Serial.println(F("\n━━━ 🧪 TEST ━━━"));
  Serial.println(F("  SELECT+× → seq test (M10-той зөрөөгүй болгож CROSS-руу нүүсэн)"));
